...
from .const import Assets, HTTPMethods, InvoiceStatus, Networks, PaidButtons
...
class AioCryptoPay(BaseClient):
    ...
    def __init__(self, token: str, network: Union[str, Networks] = Networks.MAIN_NET) -> None:
        super().__init__()
        """
    ...
    async def get_me(self) -> Profile:
        ...
        response = await self._make_request(method=method, url=url, headers=self.__headers)
    ...
    async def create_invoice(
        ...
        response = await self._make_request(method=method, url=url, params=params, headers=self.__headers)
